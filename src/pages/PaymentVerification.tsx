...
import { CheckCircle2, XCircle, Eye, Download, Search, Filter, Calendar, DollarSign, FileText, ChevronDown, ChevronUp } from 'lucide-react';
...
export function PaymentVerification() {
  ...
  async function fetchTransfers() {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*, declarations(prime_name, month, year)')
      ...
      const formatted = (data || []).map((t: any) => ({
        ...t,
        prime_name: t.declarations?.prime_name,
        month: t.declarations?.month,
        year: t.declarations?.year,
      }));
  ...
  async function handleVerify(transferId: string, status: 'verified' | 'rejected') {
    ...
    try {
      ...
      // Update declaration payment status
