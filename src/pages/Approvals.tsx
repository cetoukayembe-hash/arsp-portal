...
export function Approvals() {
  ...
  async function fetchEnterprises() {
    setLoading(true);
    const { data } = await supabase.from('enterprises').select('*').order('created_at', { ascending: false });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          {pendingCount > 0 && (
            <p className="text-sm text-amber-600 mt-1">{pendingCount} demande(s) en attente</p>
          )}
        </div>
              ...
              <div key={e.id} className="bg-white rounded-xl p-5 card-shadow">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => setSelectedEnterprise(e)} className="flex items-center gap-1 px-3 py-1.5 border border-gray-200 text-gray-600 rounded-lg text-xs hover:bg-gray-50">
                      <Eye className="w-3 h-3" />Voir
                    </button>
                    {e.status === 'pending' && (
                      <>
                        <button onClick={() => handleApprove(e.id)} className="flex items-center gap-1 px-3 py-1.5 bg-emerald-500 text-white rounded-lg text-xs hover:bg-emerald-600">
                          <CheckCircle2 className="w-3 h-3" />Approuver
                        </button>
                        <button onClick={() => { setSelectedEnterprise(e); setShowReject(true); }} className="flex items-center gap-1 px-3 py-1.5 bg-red-500 text-white rounded-lg text-xs hover:bg-red-600">
                          <XCircle className="w-3 h-3" />Rejeter
                        </button>
                      </>
                    )}
                    {e.status === 'active' && (
                      <button onClick={() => handleSuspend(e.id)} className="flex items-center gap-1 px-3 py-1.5 bg-gray-500 text-white rounded-lg text-xs hover:bg-gray-600">
                        <Clock className="w-3 h-3" />Suspendre
                      </button>
                    )}
                    {e.status === 'suspended' && (
                      <button onClick={() => handleApprove(e.id)} className="flex items-center gap-1 px-3 py-1.5 bg-emerald-500 text-white rounded-lg text-xs hover:bg-emerald-600">
                        <CheckCircle2 className="w-3 h-3" />Reactiver
                      </button>
                    )}
                  </div>
                </div>
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                <div className="bg-[#F6F9FC] rounded-lg p-4 grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div><span className="text-gray-500">Capital</span><p className="font-medium text-[#0a2540]">{selectedEnterprise.congolese_capital}%</p></div>
                ...
                {selectedEnterprise.status === 'pending' && (
                  <div className="flex gap-3 pt-2">
                    <button onClick={() => handleApprove(selectedEnterprise.id)} className="flex-1 py-2.5 bg-emerald-500 text-white rounded-lg font-semibold hover:bg-emerald-600 flex items-center justify-center gap-2">
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <button onClick={() => handleReject(selectedEnterprise.id)} disabled={!rejectReason} className="w-full py-2.5 bg-red-500 text-white rounded-lg font-semibold hover:bg-red-600 disabled:opacity-50">
                  Confirmer le rejet
                </button>
              </div>
            </div>
